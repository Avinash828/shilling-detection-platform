...
class SDLib(object):
    def __init__(self, config):
        self.trainingData = []  # training data 训练集数据
        self.testData = []  # testData 测试集数据
        self.relation = []
        self.measure = []
        self.config = config # 配置文件
        self.ratingConfig = LineConfig(config['ratings.setup']) # 评分文件配置
        self.labels = FileIO.loadLabels(config['label']) # 加载标签配置
    ...
    def execute(self): # 执行
        ...
        if self.evaluation.contains('-cv'):
            ...
            res.append(
                '         ' + labels[0] + '  ' + '    '.join(np.array(values[0:3], dtype=str).tolist()) + '   ' + str(
                    count[0]) + '\n')
            res.append(
                '         ' + labels[1] + '  ' + '    '.join(np.array(values[3:6], dtype=str).tolist()) + '   ' + str(
                    count[1]) + '\n\n')
            res.append('  avg/total   ' + '    '.join(np.array(values[6:9], dtype=str).tolist()) + '   ' + str(
                count[2]) + '\n')
            print 'Total:'
            print ''.join(res)
            # for line in lines[1:]:
